...
fn for_named_provider_missing_name_lists_available_and_points_at_config() {
    let providers = vec![named_provider("opencode-go"), named_provider("NeuralWatt")];
    let msg = match ResolvedClient::for_named_provider("openai", &providers) {
        Err(MuonError::Config(s)) => s,
        Ok(_) => panic!("expected Config error, got Ok"),
        Err(other) => panic!("expected Config, got {other:?}"),
    };
    assert!(
        msg.contains("provider 'openai' is not defined in [[providers]]"),
        "missing provider: {msg}"
    );
    assert!(
        msg.contains("Available [[providers]] names: opencode-go, NeuralWatt"),
        "available list: {msg}"
    );
    assert!(
