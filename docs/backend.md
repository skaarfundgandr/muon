...
- **Shared helpers** (`src/application/services/export_text.rs`): `soft_wrap_markdown_for_pdf(md, width) -> String` wraps markdown source at a character budget while skipping fenced code blocks; overlong tokens break on `/ ? & =` then hard-chunk at the width. Used only by the PDF exporter.
