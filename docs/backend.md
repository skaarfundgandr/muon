...
- **Shared helpers** (`src/application/services/export_text.rs`): `soft_wrap_markdown_for_pdf(md, width) -> String` wraps markdown source at a character budget for PDF layout while skipping fenced code blocks. pdf_oxide's `from_markdown` lays out one source line → one PDF line and never wraps on its own, so any source line longer than the printable text width overflows the right margin and clips. The helper pre-wraps to a safe budget and additionally (a) replaces standalone thematic-break lines (`---`, `----`, …) with a row of em-dashes that renders as a visible horizontal divider, (b) wraps ATX heading text and indents continuation lines under the heading text column so long titles don't clip the right margin, and (c) breaks overlong single tokens (URLs, paths) on `/ ? & =` then hard-chunks at the budget.
- **`MarkdownExporter`** — writes a report to `~/.local/share/muon/exports/<session_id>.md` with YAML frontmatter (title, query, created_at, source count). The YAML `title:` is the canonical file title (e.g. `plan.plan_title` for deep research, `"Shallow Research"` for shallow); the body mirrors the LLM-drafted markdown verbatim, including any leading `# Title` heading the orchestrator wrote. To prevent duplication, `citation_verifier::report_to_markdown` no longer re-injects `# {report.title}` on top of the summary — it only emits `summary` + `## sections`.
...
- **`PdfExporter`** — writes a PDF to `~/.local/share/muon/exports/<session_id>.pdf` via `pdf_oxide::api::PdfBuilder::new().title(report.title).subject(session.query).from_markdown(&body)` + `save`. The PDF body mirrors the LLM-drafted markdown (`summary` + sections + references) the same way the Markdown exporter does, then `soft_wrap_markdown_for_pdf(body, 72)` is applied so paragraphs and long reference URLs fit the page instead of clipping at the right margin. Width 72 matches the default Letter + 72 pt margin + 12 pt font text budget of ~468 pt (~78 chars/line cap; 72 leaves headroom for variable-width glyphs and bold runs). Standalone `---` divider lines are expanded into em-dash rows visible as horizontal rules; ATX heading text exceeding the budget is wrapped with continuation lines indented under the heading. The canonical file title (`report.title`) is set as PDF Info metadata via `PdfBuilder::title` and is **not** injected as a page heading; the body keeps whatever `# Title` the orchestrator drafted. The PDF body does not include the YAML frontmatter block (Markdown export is the YAML-bearing path). Reaches the TUI via the **F2** action-bar button (`[F2] Export PDF`) and via `muon export <session> pdf -o path` on the CLI; F2 has full parity with F1 Export MD (hover, click hit-target, success/error toasts). `pdf_oxide` is also the PDF→Markdown ingest source for RAG directory indexing (multi-page, see §3.9).
