...
- Config: pure settings types in `application::config` (`MuonConfig` + nested + `AgentSettings`); load/save/watch/scaffold + agent markdown parse/save + env expansion in `infrastructure::config`. TOML (`~/.config/muon/config.toml`) holds providers, pipeline knobs, display, advanced. Per-agent model/provider/temperature/max_tokens/timeout SSOT is YAML frontmatter under `agents/*.md` (Settings → Agents edits YAML on Ctrl+S).
